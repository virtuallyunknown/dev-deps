...
export const RuleUpgrades = () => {
    ...
    return (
        <>
            {upgrades.map(rule => (
                <div className="flex w-full flex-col gap-y-4 rounded-lg bg-white px-4 py-2 shadow" key={rule.upgrade.name}>
                    <div className="flex justify-between">
                        <div className="flex items-center gap-x-2">
                            <h6 className="cursor-pointer text-lg font-semibold hover:underline" >{rule.upgrade.name}</h6>
                        </div>
                        <Button text="Upgrade Rule" color="gray" size="small" onClick={() => upgradeRule(rule.upgrade)} />
                    </div>
                    <div className="flex flex-col gap-2">
                        {rule.diffs.map(diff => <Diff diff={diff} key={rule.upgrade.name + diff.type + diff.path} />)}
                    </div>
                </div>
            ))}
        </>
