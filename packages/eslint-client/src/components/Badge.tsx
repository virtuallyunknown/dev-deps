export const Badge = ({ type }: { type: 'deprecated' | 'removed' | 'recommended' | 'fixable' | 'hasSuggestions' | 'handledByTypescript' }) => {
    switch (type) {
        case 'deprecated': return <span className='rounded bg-red-100 px-2.5 py-0.5 text-xs font-medium text-red-800'>Deprecated</span>;
        case 'removed': return <span className='rounded bg-red-100 px-2.5 py-0.5 text-xs font-medium text-red-800'>Removed</span>;
